...
これらのシステムでは,膨大な数のデバイス間でデータの授受や価値の取引が自律的に行われることが期待されており,その信頼性を担保する技術としてブロックチェーンおよびスマートコントラクトが注目を集めている\cite{iot-blockchain}。
...
本研究では,スマートコントラクトの複雑な論理演算をブロックチェーンの外側（オフチェーン）で,かつデバイスに近い組込み環境で実行し,
...
第4章では作成した仕組みを使用して従来のフルオンチェーン型のシステムと比較して評価実験を行い,組込み機器への適用性を評価する。
...
またWebAssembly（Wasm）などの軽量な実行形式を用いることで,組込み機器のような計算資源が限られた環境でも動作可能である。
...
\chapter{提案手法}
\section{オフチェーン実行+オンライン同期:DTVMとブロックチェーンの接続}
本研究ではスマートコントラクトの処理をすべてブロックチェーンで実行するのではなく,オフチェーンで処理を行い,その結果のみをブロックチェーンに記録する方式を採用する。
現状DTVMはただの実行マシンであり計算処理や判定等は得意としているが,持っているデータの状態管理等はできないため,スマートコントラクト内でも比較的単純な処理を対象にしてDTVMにオフチェーンで実行させる。\\
一般的なシーケンス図をちゃんと綺麗に書いてここに載せる


\chapter{評価実験}
\section{実験概要}
\subsection{実験1:単純なケースでの検証}
実験1について簡単に説明
\subsection{実験2:複雑なユースケースにおける検証}
実験2について簡単に説明
\subsection{実験環境}
windows 11 pro\\
...
\section{実験1:単純なケースでの検証}
\subsection{実験内容}
シーケンス図やソースコードを使いながら解説
\subsection{実験結果}
フルオンチェーンとの比較結果

\section{実験2:複雑なユースケースにおける検証}
\subsection{実験内容}
シーケンス図やソースコードを使いながら解説
