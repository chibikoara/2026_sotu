...
以上のことから本研究ではオフチェーン実行向きであり,かつ実行速度も速く,Solidityのコントラクトを実行可能という点からDTVMをオフチェーン実行基盤として採用し,
DTVMで得られた実行結果をブロックチェーンへ送信し,状態として保存する接続機構を構築する。
...
すなわち,ブロックチェーンの合意層とは分離された形で,高速なスマートコントラクト実行環境としてDTVMを用いる。
...
図\ref{fig:chain}に示すように,従来のフルオンチェーンの方式と提案アーキテクチャではブロックチェーン上で実行される処理の量に違いがあることが分かる。
ブロックチェーン上で実行される処理量を削減することで,ガスコストの低減および実行負荷の軽減を図る。

このように,本研究では「実行層」と「合意・保存層」を分離した構成を採用する。
\begin{figure}[tb]
  \centering
...
この設計により,オフチェーンでの高速な計算処理と,ブロックチェーン上での結果保存を統合することが可能となる。
一方で,本設計ではオフチェーン実行部の正当性検証を行わないため,セキュリティ面での課題が残る。これについては第5章にて今後の課題として述べる。
...
従来のフルオンチェーンで実行するパターン１に対して,オフチェーン実行+オンチェーン同期型であるパターン２のほうがプログラム全体の実行時間は約0.1秒ほど長く,
メモリ使用量は約2000KB多くなった。
またパターンごとに異なる部分である仮想マシン呼び出し部分の実行時間は約0.1秒ほどパターン２の方が長くなっており,この差はプログラム全体の実行時間の差とほとんど一致していた。
...
\section{考察}
実行時間とメモリ使用量についてはパターン１と比較してパターン２の方がわずかに増加した。
...
ただガスコストについてはパターン２の方が大きく削減できている。そのためオフチェーン実行を実現することができれば
...
実験結果では,従来のフルオンチェーン型の方式と比較して本研究で提案した手法の方が実行時間とメモリ使用量がわずかに増加した。
...
オフチェーン実行の方が大きく削減できており,ガスコスト削減の観点でのオフチェーン実行+オンチェーン同期型の有用性を確認した。
\section{今後の課題}
